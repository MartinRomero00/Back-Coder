...
export const addProductToCart = async (idUser, idProduct, quantity) => {
    try {
        ...
        console.log(productExist.owner);
        console.log(userExist._id);
        if(userExist.role === "premium"){
            if(productExist.owner.toString() !== userExist._id.toString()){
...
export const validatorFormPassword = async (email, pass) => {
    try {
        const user = await getByEmail(email);
        if (user) {
            const passwordMatch = isValidPassword(pass, user);
            if (passwordMatch) {
                return user;
            }else{
                return false;
